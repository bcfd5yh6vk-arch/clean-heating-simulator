...
In these equations, \(p_{\mathrm{coal}}=1000\) yuan/ton is the scattered-coal price, \(e_{\mathrm{coal}}=23.2\) GJ/ton is coal energy content, and \(\gamma_{\mathrm{coal}}=89.0\) tCO\textsubscript{2}/TJ is the coal emission factor. Dividing by 1000 converts GJ to TJ.
...
where \(r\) is the chosen route. The constants are \(k_{\mathrm{gas}}=3000\), \(k_{\mathrm{ground}}=10000\), and \(k_{\mathrm{air}}=7000\) yuan per 100 m\textsuperscript{2}. Route-specific annual heating cost is then estimated from the initial coal baseline:
...
Here \(m\) is the consecutive non-rectification streak. The constants 40 and 15 are the base penalty weight and the additional basis-point weight per 100 m\textsuperscript{2}. If a random draw is below \(p_{\mathrm{seized}}\), the session ends in enforcement/seizure.
...
Insulation repair costs 1000 yuan per 100 m\textsuperscript{2}, then reduces annual heating demand to 62.5\% of the previous value:
...
The 0.6 yuan/m\textsuperscript{3} constant is the assumed subsidy reduction. The terminal success condition requires all of the following:
