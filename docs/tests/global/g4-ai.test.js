...
test("static 37. China Pilot stays reachable and untouched by Global AI panel", () => {
  // 本仓库 `/` 和 `/china` 都进 China Pilot。Global 的 AI 面板不得污染中国沙盘。
...
test("panel 37. 同分辅助措施变体合并为一组（D14，展示层）", () => {
  const mk = (id, rank, fitness, aux, dims) => ({
    path_id: id, rank, fitness,
    primary_tech_ids: ["ashp_ductless"], supporting_measure_ids: aux,
    dimensions: dims || { affordability: 90, climate_resilience: 80, environment: 70, practicality: 60 },
  });
  const grouped = g4.groupRankedPathsForDisplay([
    mk("bare", 1, 88.8, []),
    mk("roof", 2, 88.8, ["cool_roof"]),
    { ...mk("a2w", 3, 87.0, []), primary_tech_ids: ["a2w_hp"] },
  ]);
  assert.equal(grouped.length, 2);
  assert.equal(grouped[0].representative.path_id, "bare");
  assert.deepEqual(grouped[0].variants.map((p) => p.path_id), ["roof"]);
  assert.equal(grouped[1].variants.length, 0);

  // 综合分相同但某一维不同 → 不合并（不是纯辅助措施差异）
  const notMerged = g4.groupRankedPathsForDisplay([
    mk("x", 1, 88.8, []),
    mk("y", 2, 88.8, ["erv"], { affordability: 89, climate_resilience: 80, environment: 70, practicality: 60 }),
  ]);
  assert.equal(notMerged.length, 2);

  // 代表 = rank 最小者，即使它带辅助措施（引擎并列次序的赢家）
  const auxFirst = g4.groupRankedPathsForDisplay([
    mk("withAux", 1, 88.8, ["cool_roof"]),
    mk("bare2", 2, 88.8, []),
  ]);
  assert.equal(auxFirst[0].representative.path_id, "withAux");
});
